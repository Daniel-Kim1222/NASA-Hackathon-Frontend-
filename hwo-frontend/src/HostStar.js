...
function HostStar({ system, onClick, isSelected }) {
  ...
  // Reference to the mesh (should be declared before any condition)
  ...
  const position = [cartesian_x, cartesian_y, cartesian_z];
    ...
    <group position={scaledPosition}>
      ...
      <mesh
        ...
        onClick={(event) => {
          ...
          onClick(hostname); // Call the onClick handler with the star's name
        }}
      >
        <sphereGeometry args={[starSize * 0.99, 32, 32]} />
      ...
      <mesh>
        ...
        {/* Slightly larger than original star */}
        <meshBasicMaterial
          color={starColor}
